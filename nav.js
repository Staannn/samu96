    ...
    sidebar.innerHTML = `
      <div class="sidebar-section">
        ...
        ${sidebarLinks
          .map(
            (link) =>
              `<a href="${link.href}" class="sidebar-link${activeSection === link.href ? ' active' : ''}"><span class="icon nav-icon">${link.icon}</span>${link.label}</a>`
          )
          .join('')}
        <div class="sidebar-divider"></div>
        <div class="sidebar-label">ACCÈS RESTREINT</div>
        <a href="intranet.html" class="sidebar-link${activeSection === 'intranet.html' ? ' active' : ''}"><span class="icon nav-icon">🔒</span>Intranet</a>
      </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (document.querySelector('.topbar') || document.querySelector('.sidebar')) {
    renderNav(page);
  }
});
